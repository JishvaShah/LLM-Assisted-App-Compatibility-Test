...
class ImageProcessingAPITests(APITestCase):
    def setUp(self):
        ...
        self.image_file1 = open('/Users/chenyian261/Documents/GitHub/CS6510Project/backend/test.png', 'rb')
        self.image_file2 = open('/Users/chenyian261/Documents/GitHub/CS6510Project/backend/test.png', 'rb')
...
class ScreenshotListAPIViewTestCase(APITestCase):
    def setUp(self):
        ...
        Screenshot.objects.create(image_url="http://example.com/image1.png", flag=True, analysis_result={"flag":"true", "output_text":"There was an error."})
        Screenshot.objects.create(image_url="http://example.com/image2.png", flag=False, analysis_result={"flag":"false", "output_text":"No issues detected."})
    ...
    def test_filter_screenshots_by_date(self):
        ...
        response = self.client.get(self.url, {'start_date': str(today), 'end_date': str(today)})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        for screenshot in response.data:
            created_at = timezone.datetime.strptime(screenshot['created_at'], "%Y-%m-%dT%H:%M:%S.%fZ").date()
            self.assertEqual(created_at, today)

    def test_no_screenshots_on_invalid_date(self):
        response = self.client.get(self.url, {'start_date': '2100-01-01', 'end_date': '2100-01-01'})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class ScreenshotTestCase(TestCase):
    ...
    def test_generate_signed_url(self, mock_storage_client):
        ...
        mock_blob.generate_signed_url.return_value = 'http://mockedsignedurl.com/signed-image.png'
        ...
        mock_blob.generate_signed_url.assert_called_once_with(expiration=datetime.timedelta(minutes=15))